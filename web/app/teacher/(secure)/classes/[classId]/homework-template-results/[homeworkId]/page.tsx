...
export default async function HomeworkTemplateResultsPage({
  params,
}: {
  ...
  let templateId: HomeworkTemplateId | null = null;
  let levelLabel = "homework";
  let partLabelFor = (partId: string) => partId;

  if (homework.payload.type === "homework_template") {
    const template = getHomeworkTemplateDefinition(homework.payload.templateId);
    if (!template) notFound();
    templateId = template.id;
    levelLabel = template.level;
    partLabelFor = (partId) => homeworkTemplatePartLabel(template.id, partId);
  } else if (homework.payload.type === "graded_track") {
    const freeze = parseGradedTrackFreezeDocument(homework.payload.document);
    const template = getHomeworkTemplateDefinition(homework.payload.originTemplateId);
    if (!template || !freeze) notFound();
    templateId = template.id;
    levelLabel = homework.payload.level;
    const freezeLabels = new Map(freeze.parts.map((part) => [part.id, part.label]));
    // Primary HT1 submissions use section ids; also try sectionId from freeze parts.
    const bySectionId = new Map(freeze.parts.map((part) => [part.sectionId, part.label]));
    partLabelFor = (partId) =>
      freezeLabels.get(partId) ??
      bySectionId.get(partId) ??
      homeworkTemplatePartLabel(template.id, partId);
  } else {
    notFound();
  }
    ...
    <main className="space-y-5">
      ...
      <header className="rounded-xl border border-neutral-200 bg-white p-5">
        <p className="text-xs font-semibold uppercase tracking-wide text-neutral-500">
          {teacherClass.title} · {levelLabel} homework review
          {homework.payload.type === "graded_track" ? " · frozen graded track" : ""}
        </p>
        <h1 className="mt-1 text-2xl font-bold text-neutral-900">{homework.title}</h1>
        <p className="mt-2 text-sm text-neutral-600">
          Automatically scored parts show their results immediately. Open responses can be
          scored and given feedback below.
        </p>
      </header>

      {submissions.length === 0 ? (
        <section className="rounded-xl border border-dashed border-neutral-300 bg-white p-6 text-sm text-neutral-600">
          No reviewable submissions are stored yet.
        </section>
      ) : (
        <div className="space-y-4">
          {submissions.map((submission) => {
            const parts = Object.entries(submission.content.parts).map(
              ([partId, part]) => ({
                id: partId,
                label: partLabelFor(partId),
                ...part,
              }),
            );
            const autoParts = parts.filter((part) => part.correct !== null);
            const autoCorrect = autoParts.reduce(
              (total, part) => total + (part.correct ?? 0),
              0,
            );
            const autoTotal = autoParts.reduce((total, part) => total + part.total, 0);
            const reviewedParts = Object.values(submission.review?.grades ?? {});
            const reviewedScore = reviewedParts.reduce(
              (total, grade) => total + grade.score,
              0,
            );
            const reviewedTotal = reviewedParts.reduce(
              (total, grade) => total + grade.maxScore,
              0,
            );
            return (
              <section
              ...
              >
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    ...
                    {autoTotal || reviewedTotal ? (
                      <p className="mt-1 text-sm font-bold text-neutral-700">
                        Recorded score: {autoCorrect + reviewedScore}/
                        {autoTotal + reviewedTotal}
                      </p>
                    ) : null}
                  </div>
                ...
                <div className="mt-4 grid gap-3 lg:grid-cols-2">
                  {parts.map((part) => {
                    const recording = submission.recordings.find(
                    ...
                    return (
                      ...
                      >
                        ...
                        <dl className="mt-3 space-y-2">
                          {Object.entries(part.answers).map(([id, answer]) => (
                            <div key={id}>
                        ...
                        {recording?.url ? (
                          <audio
                            ...
                            aria-label={`${part.label} recording`}
                          />
                        ) : null}
                      </article>
                    );
