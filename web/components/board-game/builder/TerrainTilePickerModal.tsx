...
type Props = {
  open: boolean;
  selectedId?: WkeTerrainTileId;
  autotileId?: WkeTerrainTileId;
  onSelect: (tileId: WkeTerrainTileId) => void;
  onClose: () => void;
};

export function TerrainTilePickerModal({
  open,
  selectedId,
  autotileId,
  onSelect,
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="terrain-tile-picker-title"
      onClick={onClose}
    >
      <KidPanel
      ...
      >
        <header className="flex shrink-0 items-start justify-between gap-3 border-b-2 border-kid-ink/15 px-4 py-3">
          <div>
            ...
            <h2 id="terrain-tile-picker-title" className="text-lg font-extrabold text-kid-ink">
              Choose terrain art
            </h2>
            <p className="text-xs font-semibold text-kid-ink/60">
              Pick grass, sand, snow, water, or stone under this cell. Autotile suggestion is highlighted.
            </p>
          </div>
          <button
            type="button"
            ...
            <section key={group.label} className="space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
                {group.assetIds.map((assetId) => {
                  const meta = terrainTileLabel(assetId);
                  ...
                  return (
                    <button
                      ...
                      className={clsx(
                        ...
                        isAutotile && !isSelected && "ring-2 ring-emerald-400/60 ring-offset-1",
                      )}
                      title={`${meta.title}\n${meta.subtitle}\n${assetId}`}
                      onClick={() => {
                        onSelect(assetId);
                    ...
                    >
                      <div className="overflow-hidden rounded-md bg-[#3a3a3a]">
                        <TerrainTileThumbnail tileId={assetId} sizePx={64} alt={meta.title} />
                      </div>
                      ...
                      {isAutotile ?
                        <span className="text-[0.5rem] font-bold uppercase text-emerald-700">
                          Autotile
                        </span>
                      : null}
                    </button>
