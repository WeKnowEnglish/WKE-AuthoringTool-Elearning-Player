...
export function PosterSummaryGrid({ grid, variant = "poster", accentColor = "#1d4ed8" }: Props) {
  const headerClass =
    variant === "poster" ?
        ...
        {grid.rows.map((row) => (
          <Fragment key={row.label}>
            ...
            {row.cells.map((cell, cellIndex) => (
              <div key={`${row.label}-${cellIndex}`} className={cellClass}>
                {renderMark(cell, accentColor)}
              </div>
            ))}
          </Fragment>
        ))}
