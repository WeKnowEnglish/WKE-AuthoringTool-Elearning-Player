...
export function LandingTeacherWorkflow() {
  ...
  return (
    <div className="overflow-hidden rounded-[1.75rem] border-2 border-stone-200 bg-stone-50 shadow-[0_18px_45px_rgba(20,36,94,0.1)]">
      ...
      <div className="overflow-x-auto border-b border-stone-200 bg-white px-3 py-3 sm:px-5"><div className="grid min-w-[38rem] grid-cols-5 gap-2" role="tablist" aria-label="Lesson creation demo">{STAGES.map((item, index) => { const active = stage === item.id; return <button key={item.id} type="button" role="tab" aria-selected={active} disabled={!availableStages.has(item.id)} onClick={() => setStage(item.id)} className={`rounded-xl border px-3 py-2 text-left transition ${active ? "border-sky-300 bg-sky-50 text-sky-950" : "border-stone-200 bg-white text-stone-600 hover:bg-stone-50"} disabled:cursor-not-allowed disabled:opacity-40`}><span className="text-[10px] font-bold uppercase tracking-wide">Step {index + 1}</span><span className="mt-0.5 block text-sm font-semibold">{item.short}</span></button>; })}</div></div>

      <div className="min-h-[35rem] p-3 sm:p-5 lg:p-6"><div key={`${stage}-${destination}`} className="teacher-workflow-preview-motion">
        {stage === "vocabulary" ? <div className="grid gap-4 lg:grid-cols-[1.05fr_.95fr]">
          <TeacherPanel><div className="flex flex-wrap items-start justify-between gap-3 border-b border-stone-200 pb-4"><div className="min-w-0 flex-1"><label className="text-xs font-medium uppercase tracking-wide text-stone-500" htmlFor="demo-list-name">Vocabulary list</label><input id="demo-list-name" value={listName} onChange={(event) => { setListName(event.target.value); invalidate(); }} className="mt-1 w-full rounded-lg border border-stone-300 px-3 py-2 text-base font-semibold text-stone-900 outline-none focus:border-sky-400" /><p className="mt-1 text-xs text-stone-500">A1 · Media matched automatically</p></div><button type="button" onClick={() => void generateActivity()} disabled={matchingMedia || words.filter((word) => word.trim()).length < 2} className="rounded-lg bg-amber-700 px-4 py-2 text-sm font-semibold text-white hover:bg-amber-600 disabled:opacity-40">{matchingMedia ? "Finding pictures…" : "Generate activity →"}</button></div><div className="mt-4 grid gap-3 sm:grid-cols-2">{words.map((word, index) => <div key={index} className="flex items-center gap-3 rounded-xl border border-stone-200 bg-stone-50 p-3"><div className="size-12 shrink-0 overflow-hidden rounded-lg border border-stone-200 bg-white"><Image src={demoMediaUrl(word, index)} alt="" width={48} height={48} unoptimized className="h-full w-full object-cover" /></div><div className="min-w-0 flex-1"><label className="text-[10px] font-bold uppercase tracking-wide text-stone-500" htmlFor={`demo-word-${index}`}>Word {index + 1}</label><input id={`demo-word-${index}`} value={word} onChange={(event) => patchWord(index, event.target.value)} className="mt-1 w-full rounded-lg border border-stone-300 bg-white px-3 py-2 text-sm font-medium text-stone-900" /></div></div>)}</div><button type="button" onClick={() => { setWords((current) => [...current, ""]); invalidate(); }} disabled={words.length >= 6} className="mt-4 rounded-lg border border-stone-300 bg-white px-3 py-2 text-sm font-medium text-stone-700 disabled:opacity-40">+ Add another word</button></TeacherPanel>
        ...
        {stage === "quiz" && compiled ? <TeacherPanel><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-wide text-stone-500">Activity builder · {currentFormat.label}</p><h4 className="mt-1 text-lg font-semibold text-stone-900">{compiled.document.name}</h4><p className="text-sm text-stone-500">{compiled.itemCount} items · images added from Media</p></div><button type="button" onClick={() => setStage("publish")} className="rounded-lg bg-stone-900 px-4 py-2 text-sm font-medium text-white">Connect to class →</button></div><div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{words.filter(Boolean).map((word, index) => <article key={`${word}-${index}`} className="overflow-hidden rounded-xl border border-stone-200 bg-stone-50"><div className="aspect-[8/5] bg-stone-100 p-2"><Image src={compiledImageAt(index) || demoMediaUrl(word, index)} alt={`Matched illustration for ${word}`} width={640} height={400} unoptimized className="h-full w-full object-contain" /></div><div className="p-3"><p className="text-[10px] font-bold uppercase tracking-wide text-emerald-700">Media matched</p><p className="mt-1 font-semibold text-stone-900">{word}</p></div></article>)}</div></TeacherPanel> : null}
