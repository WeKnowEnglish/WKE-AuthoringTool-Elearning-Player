...
import {
  DRINK_MINIGAME_INTRO_PET_DISPLAY_SCALE,
  DRINK_MINIGAME_INTRO_PET_LAYOUT,
} from "@/lib/pet/animated-pet";
import { loadBlenderScene } from "@/lib/blender/load-scene";

const INTRO_VOICE = "Your pet is thirsty! Let's make a drink.";
...
export function PetDrinkIntro({ muted, onFinished }: Props) {
    ...
    >
      <div className="flex flex-col items-center text-center">
        <div className="relative mx-auto h-[15rem] w-full max-w-[16rem] overflow-visible">
          <div
            aria-hidden
            className="absolute inset-2 -z-10 rounded-[2.5rem] border-4 border-amber-200/80 bg-gradient-to-b from-amber-100/95 to-sky-100/90 shadow-lg"
          />
          <div
            className="pointer-events-none absolute z-10"
            style={{
              left: DRINK_MINIGAME_INTRO_PET_LAYOUT.leftPx,
              bottom: DRINK_MINIGAME_INTRO_PET_LAYOUT.bottomPx,
              transform: `translate(${DRINK_MINIGAME_INTRO_PET_LAYOUT.translateXPx}px, ${DRINK_MINIGAME_INTRO_PET_LAYOUT.translateYPx}px)`,
            }}
          >
            <AnimatedPet
              mood="playful"
              size="lg"
              displayScale={DRINK_MINIGAME_INTRO_PET_DISPLAY_SCALE}
              displayAnchor="bottom"
            />
          </div>
        </div>
        <div
          className={clsx(!exiting && "pet-drink-intro-group")}
        >
          ...
          <p className="mt-2 max-w-xs text-sm font-semibold text-kid-ink/85">
            Listen to your pet, then mix the perfect drink.
          </p>
        </div>
      </div>
