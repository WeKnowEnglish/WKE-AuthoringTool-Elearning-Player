...
export function PetMemoryResultsScreen({
  outcome,
  playerMatches,
  petMatches,
  goldEarned,
  ...
  const deltas = MEMORY_PLAY_DELTAS[outcome];
  const deltaRows = (
      ...
      <KidPanel className="relative space-y-4 border-kid-ink bg-white/95 p-4">
        <div className="flex flex-col items-center text-center">
          ...
          <p className="mt-3 text-2xl font-extrabold text-kid-ink sm:text-3xl">
            {completed ? "Great memory!" : "Stopped early"}
          </p>
          <p className="mt-1 text-sm font-semibold text-kid-ink/85">
            {completed ?
              "You matched all the pairs with your pet!"
            : "Try a full game next time."}
          </p>
          <p className="mt-2 text-sm font-bold text-kid-ink">
            Your matches: {playerMatches} · Pet matches: {petMatches}
          </p>
