...
import {
  ...
  type GamesDragMatchLessonPlayerPack,
} from "@/lib/games-drag-match/parse-games-pack";
    ...
    })),
  {
    ...
    loading: () => (
      <div className="rounded-2xl border-2 border-kid-ink/20 bg-white px-6 py-10 text-center">
        <p className="text-lg font-extrabold text-kid-ink">Loading drag match…</p>
      </div>
    ),
  },
);

const BUILTIN_PACK = parseGamesDragMatchLessonPlayerPack(bakeryDragMatch);

export function GamesDragMatchPilot() {
  const [pack, setPack] = useState<GamesDragMatchLessonPlayerPack>(BUILTIN_PACK);
  ...
  const screens = useMemo((): LessonScreenRow[] => {
    return pack.screens.map((payload, index) => ({
      id: `screen-games-drag-match-${index}`,
      lesson_id: "pilot-games-drag-match",
      order_index: index,
  ...
  async function handleImportFile(file: File) {
    ...
    try {
      const next = parseGamesDragMatchLessonPlayerPack(JSON.parse(await file.text()));
      setPack(next);
  ...
  return (
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-4 p-4">
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <h1 className="text-2xl font-extrabold text-kid-ink">
              Quizzes · Drag match (pilot)
            </h1>
            <p className="mt-1 text-sm font-semibold text-kid-ink/80">
              Studio Quiz drag-match packs play as Lesson Player drag_match screens.
            </p>
      ...
      <KidPanel>
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-xs font-semibold text-kid-ink/70">
              Drop a{" "}
              <code className="font-mono">.games-drag-match.lessonplayer.json</code> from EDU
              Studio Quiz.
            </p>
            {importNotice && (
        ...
        >
          Drop Lesson Player drag-match pack JSON here
        </div>
      </KidPanel>
      ...
      <div className="min-h-[min(75dvh,640px)]">
        <LessonPlayer
          key={generation}
          lessonId="pilot-games-drag-match"
