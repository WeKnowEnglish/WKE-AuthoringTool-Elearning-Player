...
import {
  ...
  type GamesLineMatchLessonPlayerPack,
} from "@/lib/games-line-match/parse-games-pack";
    ...
    })),
  {
    ...
    loading: () => (
      <div className="rounded-2xl border-2 border-kid-ink/20 bg-white px-6 py-10 text-center">
        <p className="text-lg font-extrabold text-kid-ink">Loading line match…</p>
      </div>
    ),
  },
);
...
export function GamesLineMatchPilot() {
  ...
  const screens = useMemo((): LessonScreenRow[] => {
    return pack.screens.map((payload, index) => ({
      id: `screen-games-line-match-${index}`,
      lesson_id: "pilot-games-line-match",
      order_index: index,
  ...
  async function handleImportFile(file: File) {
    ...
    try {
      const next = parseGamesLineMatchLessonPlayerPack(JSON.parse(await file.text()));
      setPack(next);
  ...
  return (
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-4 p-4">
      ...
      <KidPanel>
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-xs font-semibold text-kid-ink/70">
              Drop a{" "}
              <code className="font-mono">.games-line-match.lessonplayer.json</code> from EDU
              Studio Quiz.
            </p>
            {importNotice && (
        ...
        >
          Drop Lesson Player line-match pack JSON here
        </div>
      </KidPanel>
      ...
      <div className="min-h-[min(75dvh,640px)]">
        <LessonPlayer
          key={generation}
          lessonId="pilot-games-line-match"
