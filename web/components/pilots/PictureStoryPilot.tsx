...
export function PictureStoryPilot() {
      ...
      <div className="mx-auto max-w-5xl space-y-4">
        <header className="rounded-2xl border-2 border-slate-200 bg-white/95 px-4 py-3 shadow-sm">
          ...
          <p className="text-sm font-semibold text-slate-600">
            Standalone picture story — frames then questions from Activity Bank or the
            sample.
          </p>
          {notice ? (
        ...
        <PictureStoryPlayer activity={toPictureStoryPlayable(document)} />
      </div>
    </main>
  );
}
