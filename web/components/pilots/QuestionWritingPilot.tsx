...
export function QuestionWritingPilot() {
  const searchParams = useSearchParams();
  const activityId = searchParams.get("activity")?.trim() || null;
  const [document, setDocument] = useState<QuestionWritingDocument | null>(null);
      ...
      <div className="mx-auto max-w-5xl space-y-4">
        <header className="rounded-2xl border-2 border-slate-200 bg-white/95 px-4 py-3 shadow-sm">
          ...
          <p className="text-sm font-semibold text-slate-600">
            Standalone writing module — form questions from prompts, save to Activity Bank,
            assign as homework.
          </p>
          {notice ? (
        ...
        <QuestionWritingPlayer activity={toQuestionWritingPlayable(document)} />
      </div>
    </main>
  );
}
