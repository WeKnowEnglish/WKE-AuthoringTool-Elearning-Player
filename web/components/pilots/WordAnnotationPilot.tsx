...
export function WordAnnotationPilot() {
  const searchParams = useSearchParams();
  const activityId = searchParams.get("activity")?.trim() || null;
  const [document, setDocument] = useState<WordAnnotationDocument | null>(null);
      ...
      <div className="mx-auto max-w-5xl space-y-4">
        <header className="rounded-2xl border-2 border-slate-200 bg-white/95 px-4 py-3 shadow-sm">
          ...
          <p className="text-sm font-semibold text-slate-600">
            Standalone grammar module — circle adjectives, underline adverbs, save to
            Activity Bank, assign as homework.
          </p>
          {notice ? (
            <p className="mt-2 text-xs font-bold text-emerald-800">{notice}</p>
          ) : null}
        </header>
        <WordAnnotationPlayer activity={toWordAnnotationPlayable(document)} />
      </div>
    </main>
  );
}
