...
export function WhiteboardClassPanel({ classId, archived }: Props) {
  ...
  return (
    <section className="space-y-3 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
      <div>
        <h2 className="text-lg font-semibold text-slate-900">Whiteboard activity (standalone)</h2>
        <p className="text-sm text-slate-600">
          Prefer starting a{" "}
          <span className="font-semibold">Virtual Classroom</span> session above, then launching
          whiteboard from inside it. Standalone rounds still work via{" "}
          <code className="rounded bg-slate-100 px-1">/whiteboard/join</code>.
        </p>
      </div>

      {archived ? (
        <p className="text-sm text-amber-800">Unarchive the class to start a whiteboard.</p>
      ) : (
          ...
          <fieldset className="text-sm font-semibold text-slate-800">
            ...
            <div className="mt-2 flex flex-wrap gap-2">
              <button
                ...
                className={`rounded px-2 py-1 text-xs ${!worksheetPresetId ? "bg-slate-900 text-white" : "bg-slate-100"}`}
              >
                None
              </button>
              {WORKSHEET_PRESETS.map((p) => (
                <button
                  ...
                  className={`rounded px-2 py-1 text-xs ${
                    worksheetPresetId === p.id ? "bg-slate-900 text-white" : "bg-slate-100"
                  }`}
                >
                  {p.label}
                </button>
