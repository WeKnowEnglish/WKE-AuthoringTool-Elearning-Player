...
export function ClozeChoiceWorkspace() {
  const [document, setDocument] = useState<ClozeChoiceDocument>(() =>
    createSampleClozeChoiceDocument(),
  );
  ...
  const gaps = listClozeChoiceGaps(document.segments);

  const saveToBank = async () => {
    setBusy(true);
    setBanner(null);
    try {
      const valid = validateClozeChoiceDocument(document);
      const response = await fetch("/api/studio/activities", {
        ...
        body: JSON.stringify({
          id: activityId,
          format: "cloze_choice",
          pack: clozeChoiceStubPack(valid),
          authoring: valid,
          title: valid.title,
          filename: `${valid.id}.cloze-choice.json`,
          source: {
            via: "cloze_choice_workspace",
            gapCount: listClozeChoiceGaps(valid.segments).length,
          },
        }),
      });
      ...
      if (!response.ok || !payload?.ok || !payload.id) {
        throw new Error(
          payload?.error ||
            `Save failed (${response.status}). Apply migration 096 if cloze_choice is rejected.`,
        );
      }
  ...
  return (
    <div className="mx-auto flex w-full max-w-3xl flex-col gap-4 p-4 sm:p-6">
      <header>
        ...
        <h1 className="mt-1 text-2xl font-bold text-stone-900">Cloze with choices</h1>
        <p className="mt-1 text-sm text-stone-600">
          Author a short passage with 3–5 multiple-choice gaps, save to Activity Bank, then
          assign as homework. Students play a dedicated cloze shell (not Lesson Player).
          Vocab auto-compile is out of scope — start from the sample.
        </p>
      </header>
      ...
      <section className="rounded-xl border border-stone-200 bg-white p-4 shadow-sm">
        ...
        <button
          ...
          disabled={busy}
          onClick={() => {
            setDocument(createSampleClozeChoiceDocument());
            setActivityId(null);
            setBanner("Loaded school-morning sample.");
          }}
      ...
      <section className="rounded-xl border border-stone-200 bg-white p-4 shadow-sm">
        ...
        {document.passageTitle ? (
          <p className="mt-3 text-xs text-stone-500">Passage: {document.passageTitle}</p>
        ) : null}
        ...
        <ul className="mt-2 space-y-2 text-xs text-stone-700">
          {gaps.map((gap, index) => (
            <li key={gap.id}>
              <span className="font-semibold">#{index + 1}</span> {gap.correctAnswer}
              <span className="mt-0.5 block text-stone-500">
                Choices: {gap.options.join(" · ")}
              </span>
            </li>
          ))}
        </ul>
        <div className="mt-4 flex flex-wrap gap-2">
          ...
          {activityId ? (
            <Link
              href={`/pilots/cloze-choice?activity=${encodeURIComponent(activityId)}`}
