...
export function WordAnnotationWorkspace() {
  const [document, setDocument] = useState<WordAnnotationDocument>(() =>
    createSampleWordAnnotationDocument(),
  );
  ...
  const saveToBank = async () => {
    ...
    setBanner(null);
    try {
      const valid = validateWordAnnotationDocument(document);
      const response = await fetch("/api/studio/activities", {
        ...
        body: JSON.stringify({
          ...
          pack: wordAnnotationStubPack(valid),
          authoring: valid,
          title: valid.title,
          filename: `${valid.id}.word-annotation.json`,
          source: {
            via: "word_annotation_workspace",
            targetCount: countWordAnnotationTargets(valid.sentences),
          },
        }),
      });
      ...
      if (!response.ok || !payload?.ok || !payload.id) {
        throw new Error(
          payload?.error ||
            `Save failed (${response.status}). Apply migration 092 if word_annotation is rejected.`,
        );
      }
  ...
  const targetCount = countWordAnnotationTargets(document.sentences);
  ...
  return (
    <div className="mx-auto flex w-full max-w-3xl flex-col gap-4 p-4 sm:p-6">
      <header>
        ...
        <h1 className="mt-1 text-2xl font-bold text-stone-900">Word annotation</h1>
        <p className="mt-1 text-sm text-stone-600">
          Author circle-adjective / underline-adverb sentences, save to Activity Bank, then
          assign as homework. Students play a dedicated marking shell (not Lesson Player).
        </p>
      </header>
      ...
      <section className="rounded-xl border border-stone-200 bg-white p-4 shadow-sm">
        ...
        <button
          ...
          disabled={busy}
          onClick={() => {
            setDocument(createSampleWordAnnotationDocument());
            setActivityId(null);
            setBanner("Loaded Homework Template One Part 2 sample.");
          }}
        ...
        <p className="mt-2 text-xs text-stone-500">
          Vocab-list auto-compile is out of scope for this slice — edit sentences below or
      ...
      <section className="rounded-xl border border-stone-200 bg-white p-4 shadow-sm">
        ...
        <p className="mt-3 text-xs text-stone-500">
          {document.sentences.length} sentence
          {document.sentences.length === 1 ? "" : "s"} · {targetCount} target
          {targetCount === 1 ? "" : "s"}
        </p>
        <ul className="mt-2 space-y-2 text-xs text-stone-700">
          {document.sentences.map((sentence, index) => {
            const text = sentence.tokens.map((token) => token.text).join(" ");
            const roles = sentence.tokens
              .filter((token) => token.role)
              .map((token) => `${token.text} (${token.role})`)
              .join(", ");
            return (
              <li key={sentence.id}>
                <span className="font-semibold">#{index + 1}</span> {text}
                {roles ? (
                  <span className="mt-0.5 block text-stone-500">Targets: {roles}</span>
                ) : null}
              </li>
            );
          })}
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          ...
          {activityId ? (
            <Link
              href={`/pilots/word-annotation?activity=${encodeURIComponent(activityId)}`}
