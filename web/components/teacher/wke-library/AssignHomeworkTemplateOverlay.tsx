...
import { assignHomeworkTemplate } from "@/lib/actions/class-homework";
import { getHomeworkTemplateDefinition, type HomeworkTemplateId } from "@/lib/homework-templates/registry";

type ClassOption = { id: string; title: string };

export function AssignHomeworkTemplateOverlay({ open, onClose, classes, templateId = "homework-template-one" }: { open: boolean; onClose: () => void; classes: readonly ClassOption[]; templateId?: HomeworkTemplateId }) {
  const template = getHomeworkTemplateDefinition(templateId)!;
  ...
  async function submit(status: "draft" | "assigned") {
    if (status === "assigned" && template.contentStatus !== "ready") {
      setError("Add and verify the six activities before assigning this shell to students.");
      return;
    }
    setBusy(true);
    setError(null);
    const result = await assignHomeworkTemplate({
      templateId,
      classId,
      title,
      instructions,
  ...
  if (!open) return null;
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" role="dialog" aria-modal="true" aria-label={`Assign ${template.title}`} onClick={onClose}>
    <div className="w-full max-w-lg rounded-2xl border border-stone-200 bg-white shadow-xl" onClick={(event) => event.stopPropagation()}>
      <header className="flex items-start justify-between gap-3 border-b border-stone-200 p-4"><div><h2 className="text-lg font-bold text-stone-900">Assign as homework</h2><p className="text-sm text-stone-600">{template.title} · 6 parts · {template.level === "secondary" ? "Secondary" : "Primary"}</p></div><button type="button" onClick={onClose} className="rounded-lg border border-stone-300 px-3 py-1.5 text-sm font-semibold">Close</button></header>
      <div className="space-y-4 p-4">
        {success ? <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-950"><p className="font-bold">{success.status === "assigned" ? "Assigned" : "Saved as draft"}: {success.title}</p><div className="mt-3 flex gap-2"><Link href={`/teacher/classes/${success.classId}`} className="rounded-lg bg-emerald-800 px-3 py-2 font-semibold text-white">Open class</Link><button type="button" onClick={onClose} className="rounded-lg border border-emerald-300 bg-white px-3 py-2 font-semibold">Done</button></div></div> : <>
          {template.contentStatus === "shell" ? <p className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm font-semibold text-amber-950">The assignment pipeline is ready, but the activity layout is still a shell. You can prepare a class draft now; student assignment unlocks when the activities are added.</p> : null}
