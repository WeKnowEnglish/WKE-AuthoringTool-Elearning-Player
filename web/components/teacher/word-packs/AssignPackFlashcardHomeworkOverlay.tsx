...
import { assignPackFlashcardSetAsHomework } from "@/lib/actions/class-homework";
...
export function AssignPackFlashcardHomeworkOverlay({
  open,
  onClose,
  setId,
  ...
  async function submit(status: "draft" | "assigned") {
    ...
    setBusy(true);
    try {
      const result = await assignPackFlashcardSetAsHomework({
        setId,
        classId,
        title,
        instructions,
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="min-h-0 flex-1 space-y-3 overflow-auto px-4 py-4">
          {success ? (
            ...
            <p className="rounded border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-950">
              Create a class first, then assign this flashcard set.
            </p>
          ) : cardCount < 1 ? (
            <p className="rounded border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-950">
              This set has no cards, so it can’t be assigned yet.
            </p>
          ) : (
            <>
              ...
              {packId ? (
                <div className="rounded border border-neutral-200 bg-neutral-50 px-3 py-2 text-xs text-neutral-700">
                  {packLinkedHere ? (
                    ...
                    <p>
                      This pack isn’t linked to a class yet — required for the class Homework
                      flashcards list.
                    </p>
                  )}
                  {!packLinkedHere ? (
                ...
                <p className="text-xs text-amber-800">
                  This set has no source pack — students can still open it if assigned, but it won’t
                  appear in the class flashcards dropdown.
                </p>
              )}
        ...
        {!success && classes.length > 0 && cardCount > 0 ? (
