...
import type { PackQuizLetterSheetRow } from "@/lib/vocabulary/pack-quiz";
...
export function PackQuizLetterSheetTable({
  rows,
  readOnly = true,
  onChangeRow,
  onDeleteRow,
      ...
      <table className="min-w-[48rem] w-full border-collapse text-left text-sm">
        <thead className="sticky top-0 bg-neutral-100 text-xs font-semibold uppercase tracking-wide text-neutral-600">
          <tr>
            ...
            <th className="border-b border-neutral-200 px-2 py-2 min-w-[10rem]">Prompt</th>
            <th className="border-b border-neutral-200 px-2 py-2 min-w-[8rem]">Target word</th>
            <th className="border-b border-neutral-200 px-2 py-2 min-w-[14rem]">Prompt image</th>
            <th className="border-b border-neutral-200 px-2 py-2 min-w-[10rem]">
              Extra accepted
            </th>
            {!readOnly ? (
        ...
        <tbody>
          {rows.map((row, index) => (
            <tr key={row.id} className="align-top odd:bg-white even:bg-neutral-50/60">
              ...
              <td className="border-b border-neutral-100 px-2 py-2">
                {readOnly ? (
                  <span className="font-medium text-emerald-900">{row.targetWord}</span>
                ) : (
                  <input
                    className={`${cellInputClass} font-medium text-emerald-900`}
                    value={row.targetWord}
                    aria-label={`Target word for question ${index + 1}`}
                    onChange={(e) =>
                      onChangeRow?.(row.id, { targetWord: e.target.value })
                    }
                  />
                )}
              </td>
              ...
              <td className="border-b border-neutral-100 px-2 py-2">
                {readOnly ? (
                  <span className="text-neutral-700">{row.extraAccepted.trim() || "—"}</span>
                ) : (
                  <input
                    className={cellInputClass}
                    value={row.extraAccepted}
                    placeholder="optional, comma-separated"
                    aria-label={`Extra accepted spellings for question ${index + 1}`}
                    onChange={(e) =>
                      onChangeRow?.(row.id, { extraAccepted: e.target.value })
                    }
                  />
                )}
              </td>
