...
import type { PackQuizSentenceSheetRow } from "@/lib/vocabulary/pack-quiz";
...
export function PackQuizSentenceSheetTable({
  rows,
  readOnly = true,
  onChangeRow,
  onDeleteRow,
      ...
      <table className="min-w-[48rem] w-full border-collapse text-left text-sm">
        <thead className="sticky top-0 bg-neutral-100 text-xs font-semibold uppercase tracking-wide text-neutral-600">
          <tr>
            ...
            <th className="border-b border-neutral-200 px-2 py-2 min-w-[16rem]">Sentence</th>
          ...
          {rows.map((row, index) => (
            <tr key={row.id} className="align-top odd:bg-white even:bg-neutral-50/60">
              ...
              <td className="border-b border-neutral-100 px-2 py-2">
                {readOnly ? (
                  <span className="whitespace-pre-wrap font-medium text-neutral-900">
                    {row.sentence}
                  </span>
                ) : (
                  <textarea
                    className={`${cellInputClass} min-h-[2.5rem] resize-y font-medium`}
                    rows={2}
                    value={row.sentence}
                    aria-label={`Sentence for question ${index + 1}`}
                    onChange={(e) => onChangeRow?.(row.id, { sentence: e.target.value })}
                  />
                )}
              </td>
              <td className="border-b border-neutral-100 px-2 py-2">
                {readOnly ? (
                  <span className="whitespace-pre-wrap text-neutral-700">{row.bodyText}</span>
                ) : (
                  <textarea
                    ...
                    onChange={(e) => onChangeRow?.(row.id, { bodyText: e.target.value })}
                  />
                )}
              </td>
              <td className="border-b border-neutral-100 px-2 py-2">
