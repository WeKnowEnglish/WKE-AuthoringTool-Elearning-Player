...
import type { DailyQuestId } from "@/lib/teststartpage/daily-quests";
import {
  DAILY_CHEST_GOLD,
  DAILY_CHEST_XP,
...
type Props = {
  ...
  embedded?: boolean;
  className?: string;
  /** When set, only these quests count toward chest (hub vs test-start). */
  questIds?: readonly DailyQuestId[];
};
...
export function DailyQuestsPanel({
  ...
  const model = hydrated ? getDailyQuestUiRows(undefined, { questIds }) : null;
  const goalCount = model?.rows.length ?? questIds?.length ?? 5;

  function handleOpenChest() {
    playSfx("tap", muted);
    const ok = openDailyTreasureChest(undefined, questIds);
    if (ok) {
      ...
      <div className="border-t-2 border-kid-ink/20 pt-3">
        ...
        {model.chestLootClaimed ?
          <p className="mt-1 text-sm font-semibold text-kid-ink/80">
            Opened today. Come back tomorrow for new quests.
          </p>
        : model.chestOpenable ?
        ...
        : (
          <p className="mt-1 text-sm font-semibold text-kid-ink/80">
            Finish all {goalCount} goals today to unlock the chest.
          </p>
        )}
      </div>
    </div>
