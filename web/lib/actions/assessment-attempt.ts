...
export async function savePrimaryA2AssessmentAttempt(input: {
  ...
  try {
    ...
    const { data: homework, error: homeworkError } = await supabase
      ...
      .eq("id", homeworkId)
      .maybeSingle();
    if (homeworkError || !homework) return { ok: false, error: homeworkError?.message ?? "Assessment not found." };
    ...
    if (homework.status !== "assigned") {
      return { ok: false, error: homework.status === "closed" ? "This assessment is closed." : "This assessment is not assigned yet." };
    }
    ...
    const enrolled = ((memberships ?? []) as Array<{ class_id: string }>).some((row) => row.class_id === homework.class_id);
    ...
    const { data: existing, error: existingError } = await supabase
      ...
      .maybeSingle();
    if (existingError && !/class_assessment_attempts|schema cache|does not exist/i.test(existingError.message)) {
      return { ok: false, error: existingError.message };
    }
    if (existing?.status === "submitted") return { ok: false, error: "This assessment has already been submitted." };

    const responses = sanitizeAssessmentResponses(PRIMARY_A2_ASSESSMENT_PILOT, input.responses);
    if (input.submit) {
      const expectedRecordings = listAssessmentParts(PRIMARY_A2_ASSESSMENT_PILOT).flatMap((part) => {
        if (!(part.kind === "speaking_picture_differences" || part.kind === "speaking_question_exchange" || part.kind === "speaking_picture_story")) return [];
      ...
      if (expectedRecordings.length) {
        const { data: recordingRows, error: recordingsError } = await supabase.from("assessment_speaking_recordings")
          .select("id, part_id, response_id")
          .eq("homework_id", homeworkId)
          .eq("student_id", user.id)
          .in("id", expectedRecordings.map((item) => item.id));
        if (recordingsError) return { ok: false, error: /assessment_speaking_recordings|schema cache|does not exist/i.test(recordingsError.message)
          ? "Speaking submissions are not available until migration 100 is applied."
          : recordingsError.message };
        const valid = new Set((recordingRows ?? []).map((row) => `${row.id}:${row.part_id}:${row.response_id}`));
        if (expectedRecordings.some((item) => !valid.has(`${item.id}:${item.partId}:${item.responseId}`))) {
          return { ok: false, error: "One of the speaking recordings could not be verified. Please record and save that part again." };
        }
      }
    }
    const progress = assessmentProgress(PRIMARY_A2_ASSESSMENT_PILOT, responses);
    const validPartIds = new Set(listAssessmentParts(PRIMARY_A2_ASSESSMENT_PILOT).map((part) => part.id));
    const activePartId = validPartIds.has(input.activePartId)
      ? input.activePartId
      : listAssessmentParts(PRIMARY_A2_ASSESSMENT_PILOT)[0]?.id ?? "";
    const now = new Date().toISOString();
    const status = input.submit ? "submitted" : "in_progress";
    const startedAt = typeof existing?.started_at === "string" ? existing.started_at : now;
    const { data: saved, error: saveError } = await supabase
      .from("class_assessment_attempts")
      .upsert({
        homework_id: homeworkId,
        student_id: user.id,
        definition_id: PRIMARY_A2_ASSESSMENT_PILOT.id,
        content_version: PRIMARY_A2_ASSESSMENT_PILOT.contentVersion,
        status,
      ...
      .single();
    if (saveError || !saved) {
      return { ok: false, error: /class_assessment_attempts|schema cache|does not exist/i.test(saveError?.message ?? "")
    ...
    return { ok: true, attempt: {
      ...
      responses: sanitizeAssessmentResponses(PRIMARY_A2_ASSESSMENT_PILOT, saved.responses),
    ...
    return { ok: false, error: error instanceof Error ? error.message : "Could not save the assessment." };
  }
}
