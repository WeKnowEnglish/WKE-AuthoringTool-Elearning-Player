...
export type GamesFillBlanksScreen = Extract<
  ScreenPayload,
  { type: "interaction"; subtype: "fill_blanks" }
>;
...
/** Parse a Studio Quiz cloze / fill-blanks export into typed LP screens. */
export function parseGamesFillBlanksLessonPlayerPack(
  raw: unknown,
): GamesFillBlanksLessonPlayerPack {
  if (isRecord(raw) && isRecord(raw.interaction) && raw.interaction.type === "games") {
    throw new Error(
      "This looks like a Studio authoring document. Export for Lesson Player first, then drop the .games-fill-blanks.lessonplayer.json file.",
    );
  }
  ...
  if (raw.format !== "fill_blanks") {
    throw new Error('This pilot only supports format "fill_blanks".');
  }
  ...
  const screens = raw.screens.map((screen, index) => {
    ...
    if (parsed.subtype !== "fill_blanks") {
      throw new Error(`Screen ${index + 1} must be subtype fill_blanks.`);
    }
    return parsed;
  });
