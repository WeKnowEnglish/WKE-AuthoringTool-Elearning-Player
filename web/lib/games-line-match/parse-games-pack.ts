...
export type GamesLineMatchScreen = Extract<
  ScreenPayload,
  { type: "interaction"; subtype: "line_match" }
>;

export type GamesLineMatchLessonPlayerPack = {
  version: 1;
...
/** Parse a Studio Quiz line-match export into typed LP screens. */
export function parseGamesLineMatchLessonPlayerPack(
  raw: unknown,
): GamesLineMatchLessonPlayerPack {
  if (isRecord(raw) && isRecord(raw.interaction) && raw.interaction.type === "games") {
    throw new Error(
      "This looks like a Studio authoring document. Export for Lesson Player first, then drop the .games-line-match.lessonplayer.json file.",
    );
  }
  ...
  if (raw.format !== "line_match") {
    throw new Error('This pilot only supports format "line_match".');
  }
  ...
  const screens = raw.screens.map((screen, index) => {
    ...
    if (parsed.subtype !== "line_match") {
      throw new Error(`Screen ${index + 1} must be subtype line_match.`);
    }
    return parsed;
  });
