...
export type GamesSentenceScrambleScreen = Extract<
  ScreenPayload,
...
export type GamesSentenceScrambleLessonPlayerPack = {
  ...
  screens: GamesSentenceScrambleScreen[];
};
...
/** Parse a Studio Quiz sentence scramble export into typed LP screens. */
export function parseGamesSentenceScrambleLessonPlayerPack(
  raw: unknown,
): GamesSentenceScrambleLessonPlayerPack {
  if (isRecord(raw) && isRecord(raw.interaction) && raw.interaction.type === "games") {
    throw new Error(
      "This looks like a Studio authoring document. Export for Lesson Player first, then drop the .games-sentence-scramble.lessonplayer.json file.",
    );
  }
  ...
  if (raw.format !== "sentence_scramble") {
    throw new Error('This pilot only supports format "sentence_scramble".');
  }
  ...
  const screens = raw.screens.map((screen, index) => {
    ...
    if (parsed.subtype !== "drag_sentence") {
      throw new Error(`Screen ${index + 1} must be subtype drag_sentence.`);
    }
    return parsed;
  });
