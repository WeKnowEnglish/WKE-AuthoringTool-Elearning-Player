...
export type GamesTrueFalseScreen = Extract<
  ScreenPayload,
...
/** Parse a Studio Quiz true/false export into typed LP screens. */
export function parseGamesTrueFalseLessonPlayerPack(
  raw: unknown,
): GamesTrueFalseLessonPlayerPack {
  if (isRecord(raw) && isRecord(raw.interaction) && raw.interaction.type === "games") {
    throw new Error(
      "This looks like a Studio authoring document. Export for Lesson Player first, then drop the .games-true-false.lessonplayer.json file.",
    );
  }
  ...
  if (raw.format !== "true_false") {
    throw new Error('This pilot only supports format "true_false".');
  }
  ...
  const screens = raw.screens.map((screen, index) => {
    ...
    if (parsed.subtype !== "true_false") {
      throw new Error(`Screen ${index + 1} must be subtype true_false.`);
    }
    return parsed;
  });
