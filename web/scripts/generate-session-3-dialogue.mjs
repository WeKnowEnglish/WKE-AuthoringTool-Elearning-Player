...
const dialoguePath = path.join(webRoot, "content", "voices", "session-3-dialogue-draft.md");
...
const generatedModulePath = path.join(webRoot, "lib", "curriculum", "session-3-dialogue.generated.ts");
...
function parseDialogue(markdown) {
  ...
  const duplicateIds = clips.filter((clip, index) => clips.findIndex((item) => item.id === clip.id) !== index);
  if (duplicateIds.length) throw new Error(`Duplicate dialogue IDs: ${duplicateIds.map((clip) => clip.id).join(", ")}`);
  return clips;
}
...
async function generateClip({ apiKey, config, clip, overwrite = false }) {
  ...
  const outputDir = path.join(audioRoot, speakerSlug(clip.speaker), "session-3");
...
async function writeManifests({ clips, config }) {
  const entries = Object.fromEntries(clips.map((clip) => {
    ...
    return [clip.id, {
      ...
      text: clip.text,
      audioUrl: `/curriculum/grade-4-movers/characters/audio/${slug}/session-3/${clip.id}.${config.format}?v=${encodeURIComponent(character.protocol)}`,
      playbackRate: character.playbackRate ?? 1,
    }];
  }));
  const manifest = {
    session: "grade-4-unit-1-session-3",
    model: config.model,
  ...
  const manifestDir = path.join(audioRoot, "session-3");
  await mkdir(manifestDir, { recursive: true });
  await writeFile(path.join(manifestDir, "manifest.json"), `${JSON.stringify(manifest, null, 2)}\n`, "utf8");
  const source = `// Generated by scripts/generate-session-3-dialogue.mjs.\n// Edit content/voices/session-3-dialogue-draft.md, then regenerate.\n\nexport const SESSION_3_DIALOGUE = ${JSON.stringify(entries, null, 2)} as const;\n\nexport type Session3DialogueId = keyof typeof SESSION_3_DIALOGUE;\n`;
...
async function main() {
  ...
  if (clips.length !== 47) throw new Error(`Expected 47 approved clips, but parsed ${clips.length}.`);
  ...
  console.log(`Validated ${clips.length} Session 3 clips across ${new Set(clips.map((clip) => clip.speaker)).size} voices.`);
  if (dryRun) return;
  const apiKey = process.env.OPENAI_API_KEY?.trim();
  if (!apiKey) throw new Error("OPENAI_API_KEY is not configured in web/.env.local.");
  const clipsToGenerate = requestedSpeakers ? clips.filter((clip) => requestedSpeakers.has(clip.speaker)) : clips;
