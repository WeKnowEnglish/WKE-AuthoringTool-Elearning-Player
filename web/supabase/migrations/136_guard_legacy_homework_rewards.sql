-- Prevent homework completed before the reward ledger rollout from earning a
-- first-completion bonus when the student merely replays it afterward.
...
begin
  ...
  select exists (select 1 from public.class_homework_completions where homework_id = p_homework_id and student_id = v_student_id)
  ...
  on conflict (homework_id, student_id) do update
    set questions_total = greatest(public.class_homework_completions.questions_total, excluded.questions_total), updated_at = now()
  returning * into v_completion;

  if not v_was_completed then
    v_receipt := public.apply_primary_reward(
      'primary:homework:' || p_homework_id::text, 'homework_completion', p_homework_id::text,
      'assigned_homework', jsonb_build_object('classId', v_homework.class_id)
    );
  else
