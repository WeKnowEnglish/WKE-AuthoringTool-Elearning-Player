-- Private speaking recordings for homework collection speaking_prompt parts.

create table if not exists public.homework_collection_speaking_recordings (
...
create index if not exists homework_collection_speaking_homework_idx
  on public.homework_collection_speaking_recordings(homework_id, student_id);

alter table public.homework_collection_speaking_recordings enable row level security;
...
create policy "homework_collection_speaking_teacher_select"
  on public.homework_collection_speaking_recordings for select
  to authenticated
  using (
    exists (
...
create policy "homework_collection_voice_student_read"
  on storage.objects for select
  to authenticated
  using (
    bucket_id = 'voice_submissions'
    and (storage.foldername(name))[1] = 'homework-collection'
    and (storage.foldername(name))[3] = auth.uid()::text
  );

create policy "homework_collection_voice_student_delete"
...
create policy "homework_collection_voice_teacher_read"
  on storage.objects for select
  to authenticated
  using (
    bucket_id = 'voice_submissions'
...
grant select, insert, update on public.homework_collection_speaking_recordings to authenticated;
